...
def run_tests(original: dict, simulated: dict, proposal) -> list[TestResult]:
    ...
    results.append(TestResult(
        ...
        passed=unchanged,
        detail="All twin fields other than 'sensors' are identical to the original."
               if unchanged else "One or more unrelated fields changed unexpectedly.",
    ))
    ...
    if proposal.change_type == "add_twin_field":
        ...
        results.append(TestResult(
            ...
            passed=present,
            detail=f"sensors.{field_name} == {expected!r}" if present
    ...
    results.append(TestResult(name="json_serializable", passed=serializes, detail=detail))
