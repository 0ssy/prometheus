...
def analyze_boot_chain(
    ...
    if signature is None or public_key_bytes is None:
        logger.info("Boot chain analysis: no signature/public key provided — status=unknown")
        return BootChainResult(
            status="unknown", firmware_hash=firmware_hash,
    ...
    try:
        ...
        return BootChainResult(status="unknown", firmware_hash=firmware_hash, detail=f"Malformed signature or key: {e}")

    if valid:
        logger.info("Boot chain analysis: signature valid")
        return BootChainResult(status="valid", firmware_hash=firmware_hash, detail="Signature verified against provided public key.")

    logger.warning("Boot chain analysis: signature INVALID")
    return BootChainResult(status="invalid", firmware_hash=firmware_hash, detail="Signature does not match firmware under provided public key.")
