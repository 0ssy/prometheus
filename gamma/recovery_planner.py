...
def plan_recovery(device_id: str, boot_chain_status: str, partition_scheme: str) -> RecoveryPlan:
    steps = []

    if boot_chain_status == "invalid":
        steps.append(RecoveryStep(
            order=1, action="halt_and_report",
        ...
        steps.append(RecoveryStep(
            order=2, action="official_reflash_guide",
        ...
        steps.append(RecoveryStep(
            order=1, action="gather_more_info",
        ...
        steps.append(RecoveryStep(
            order=1, action="no_action_needed",
    ...
    if partition_scheme == "unknown":
        steps.append(RecoveryStep(
            order=len(steps) + 1, action="partition_table_repair_guide",
    ...
    logger.info(f"Recovery plan generated for {device_id}: {scenario} -> {len(steps)} step(s)")
