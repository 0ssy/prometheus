...
impl HttpProvider {
    ...
    pub fn gemini(api_key: impl Into<String>) -> Arc<dyn Provider> {
        // Google exposes an OpenAI-compatible surface at /v1beta/openai.
        Self::with_keywords(
            "gemini",
            "Gemini",
            ProviderKind::Gemini,
    ...
    fn prepare(
        ...
        match self.style {
            ...
            ApiStyle::Anthropic => {
                ...
                // Anthropic nests text in content[].text.
                let extractor = json!({
...
impl Provider for HttpProvider {
    ...
    async fn list_models(&self) -> Result<Vec<String>, AetherError> {
        // Anthropic has no list endpoint; return the declared model set.
