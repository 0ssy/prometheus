...
impl ModelRouter {
    ...
    pub fn select(
        ...
        // Explicit request always wins when present.
        ...
        match capability {
            ...
            Some(Capability::Cheap) | Some(Capability::Fast) => {
                // Prefer local when available (cheaper, lower latency); else cloud.
                local
                    .first()
                    .or_else(|| cloud.first())
                    .map(|(id, _)| id.clone())
            ...
            Some(Capability::Reasoning) => {
                // Reasoning favors capable cloud models; fall back to any.
                cloud
                    .first()
                    .or_else(|| local.first())
                    .map(|(id, _)| id.clone())
