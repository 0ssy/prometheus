...
use ai_runtime::lm_studio::LmStudioProvider;
use ai_runtime::provider::Provider;
use ai_runtime::types::ChatRequest;
use std::sync::Arc;
...
/// Live call against a real LM Studio instance. Ignored unless you have LM
/// Studio running on the default port; CI only runs the mocked tests above.
#[tokio::test]
#[ignore]
async fn live_lm_studio_chat() {
    let provider = LmStudioProvider::default_local();
    assert!(provider.health().await.ok, "LM Studio must be running");
    let resp = provider
        .chat(ChatRequest::from_prompt("", "ping"))
        .await
        .expect("live chat");
...
async fn check_runtime_aggregates_providers_and_backend() {
    use ai_runtime::health::check_runtime;
    use ai_runtime::ProviderManager;
    ...
    manager.register(Arc::new(LmStudioProvider::new(
        "LM Studio",
